...
/** Inline token in URL: deflate+base64url, or raw base64url. */
const INLINE_TOKEN_Z = "z1.";
const INLINE_TOKEN_U = "u1.";
/** Soft limit for copied link length. */
const MAX_ATTENDEE_LINK_CHARS = 250_000;
...
function buildAttendeeLinkWithToken(encodedToken) {
  const u = new URL(window.location.href);
  u.hash = `#/attendee?t=${encodeURIComponent(encodedToken)}`;
  return u.toString();
}
...
async function encodeShareTokenForUrl(payload) {
  const json = JSON.stringify(payload);
  const bytes = new TextEncoder().encode(json);
  if (typeof CompressionStream !== "undefined") {
    const stream = new Blob([bytes]).stream().pipeThrough(new CompressionStream("deflate"));
    const deflated = new Uint8Array(await new Response(stream).arrayBuffer());
    return INLINE_TOKEN_Z + uint8ToBase64url(deflated);
  }
  if (bytes.length > 384_000) {
    throw new Error("Agenda is too large to pack into a link.");
  }
  return INLINE_TOKEN_U + uint8ToBase64url(bytes);
}

async function decodeShareTokenFromUrl(encoded) {
  const s = String(encoded).trim();
  if (s.startsWith(INLINE_TOKEN_U)) {
    const raw = base64urlToUint8(s.slice(INLINE_TOKEN_U.length));
    return JSON.parse(new TextDecoder().decode(raw));
  }
  if (s.startsWith(INLINE_TOKEN_Z)) {
    if (typeof DecompressionStream === "undefined") {
      throw new Error("This browser cannot unpack this link.");
    }
    const deflated = base64urlToUint8(s.slice(INLINE_TOKEN_Z.length));
    const stream = new Blob([deflated]).stream().pipeThrough(new DecompressionStream("deflate"));
    const out = await new Response(stream).arrayBuffer();
    return JSON.parse(new TextDecoder().decode(out));
  }
  throw new Error("Invalid or outdated attendee link.");
}

/**
 * @returns {Promise<{ kind: "ok" | "error" | "info"; text: string } | null>}
 */
async function maybeApplySharedAgendaFromUrl(params) {
  const raw = params.get("t");
  if (!raw?.trim()) return null;
  try {
    const payload = await decodeShareTokenFromUrl(raw);
    assertValidSharePayload(payload);
    saveState(savedStateFromSharePayload(payload));
    replaceHashAttendeeOnly();
    return { kind: "ok", text: "Agenda loaded from the attendee link." };
  } catch (err) {
    replaceHashAttendeeOnly();
    return { kind: "error", text: err.message || "Could not read the attendee link." };
  }
}
...
function renderPresenterPreview(items, sessions, presenterTz) {
  ...
  if (!sessions || !sessions.length) {
    ...
    const copyLinkBtn = document.getElementById("btn-copy-attendee-link");
...
function renderAttendeeAgenda(attendeeTz) {
  ...
  if (!state || !state.sessions || !state.sessions.length) {
    host.innerHTML =
      '<p class="empty-state">No agenda yet. Open the presenter’s <strong>attendee link</strong>, use <strong>Load agenda.json</strong>, or publish on this device (max 3 calendar days).</p>';
    return;
  }
...
function wirePresenterOnce() {
  ...
  document.getElementById("btn-copy-attendee-link")?.addEventListener("click", async () => {
    ...
    try {
      const encoded = await encodeShareTokenForUrl(payload);
      if (encoded.length > MAX_ATTENDEE_LINK_CHARS) {
        throw new Error("Agenda too large for a link. Use “Download agenda.json”.");
      }
      const url = buildAttendeeLinkWithToken(encoded);
...
function setAttendeeRouteBanner(notice) {
  ...
  const mod =
    notice.kind === "error"
      ...
      : notice.kind === "info"
        ? " attendee-banner--info"
        : " attendee-banner--success";
  banner.className = "attendee-banner" + mod;
  banner.setAttribute("role", notice.kind === "error" ? "alert" : "status");
}
